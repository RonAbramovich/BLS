...
- P = (3, 2)
- -P = (3, -2 mod 5) = (3, 3)
- Verification: 3^2 = 9 ≡ 4 and 3^3 + 2 = 29 ≡ 4 ✓

---

### 4.2 Point Addition (Distinct Points)

**Case**: Adding two different points P₁ = (x₁, y₁) and P₂ = (x₂, y₂) where x₁ ≠ x₂.

**Geometric Interpretation**: Draw a line through P₁ and P₂. This line intersects the curve at a third point. The sum P₁ + P₂ is the reflection of this third point across the x-axis.
...
Add(P₁, P₂):
    ...
    if P₂ = O: return P₁
    if x₁ = x₂:
        if y₁ = -y₂: return O  // vertical line
        else: return Double(P₁) // same point
    
    // Compute slope λ
    λ = (y₂ - y₁) / (x₂ - x₁)  // field division
...
**Example** (F_5, curve y^2 = x^3 + 2):
Adding P₁ = (2, 0) and P₂ = (3, 2):
- λ = (2 - 0) / (3 - 2) = 2 / 1 = 2
- x₃ = 2^2 - 2 - 3 = 4 - 5 = -1 ≡ 4 (mod 5)
- y₃ = 2(2 - 4) - 0 = -4 ≡ 1 (mod 5)
- Result: (4, 1)
...
- Denominator: 2y = 4
- Inverse of 4 in F_5 is 4 (since 4 × 4 = 16 ≡ 1)
- λ = 2 × 4 = 8 ≡ 3 (mod 5)
- x₃ = 3^2 - 2(3) = 9 - 6 = 3
- y₃ = 3(3 - 3) - 2 = -2 ≡ 3 (mod 5)
- Result: 2P = (3, 3) = -P ✓
...
- x=0: z=2, 2^2=4≠1 → non-residue → 0 points
- x=1: z=3, 3^2=4≠1 → non-residue → 0 points
- x=2: z=10≡0 → one point (2, 0)
- x=3: z=29≡4, 4^2=1 → residue → two points (3, 2) and (3, 3)
- x=4: z=66≡1, 1^2=1 → residue → two points (4, 1) and (4, 4)
...
**Example** (F_5, curve with group order 6 = 2 × 3):
For P = (3, 2):
- Start: ord = 6
- Test factor 2: (6/2) × P = 3P = O ✓ → ord = 3
- Test factor 3: (3/3) × P = 1P ≠ O ✗ → ord stays 3
- Result: Order of P is 3

For P = (2, 0):
- Start: ord = 6
- Test factor 2: (6/2) × P = 3P ≠ O (actually it equals O since 2P = O) → ord = 3
- Wait, let me recalculate: 2P = O for this point
- Test factor 2: (6/2) × P = 3P = O → ord = 3
- Test factor 3: (3/3) × P = P ≠ O → ord stays 3
- Actually, since 2P = O, we need: ord = 6, test 6/2=3: 3P=P+2P=P+O=P≠O, stays 6
- Test 6/3=2: 2P=O ✓ → ord = 2
- Result: Order of (2,0) is 2 ✓
